...
// Extracted background elements for better organization
...
// Extracted floating globes for better organization
...
// Extracted gradients for better organization
...
export default function Page() {
  ...
  useEffect(() => {
    // Only redirect if session is loaded (not loading) and user is not admin
    if (status !== "loading" && (!session?.user || session.user.role !== "ADMIN")) {
      router.push("/login");
    }
  }, [session, router, status]);
  
  // If still loading the session, could show a loading state
  if (status === "loading") {
